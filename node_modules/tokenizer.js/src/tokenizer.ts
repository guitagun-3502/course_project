...
import {doAsyncMacrotask} from '../../../base/utils/ts-utils';
import {
...
export class Tokenizer {
        ...
        let keyups = Rx.Observable
            ...
            .switchMap(
                (inputHtml: string, idx: number) => {
                    ...
                    return Rx.Observable.timer(typingDebounce)
                        .flatMap(() => doAsyncMacrotask(() => {
    ...
    private setupKeydown() {
        ...
        let handleKeyPressesOnSeparator = _.debounce((e: KeyboardEvent) => {
    ...
    private setupHover() {
        if(_.isFunction(this.config.onMouseOverToken)) {
            let mouseoverSubscription = Rx.Observable
                ...
                .filter(([tokenIndex, e]) => _.isFinite(tokenIndex))
                .subscribe(([tokenIndex, e]) => {
                    this.config.onMouseOverToken(tokenIndex, e);
                });
            this.subscriptions.push(mouseoverSubscription);
        }
        if(_.isFunction(this.config.onMouseOutTokenizer)) {
            let mouseoutSubscription = Rx.Observable
    ...
    private setupMousedown() {
        let mouseDownSubscription = Rx.Observable
    ...
    private setupBlur() {
        let blurSubscription = Rx.Observable
    ...
    private setupPaste() {
        let pasteSubscription = Rx.Observable
    ...
    private setupResizingWrapCheck() {
        let resizeSubscription = Rx.Observable
